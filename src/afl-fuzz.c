...
int main(int argc, char **argv_orig, char **envp) {
  ...
  afl->argv = use_argv;
  afl->fsrv.trace_bits =
      afl_shm_init(&afl->shm, afl->fsrv.map_size, (afl->bb_map_size + 7) / 8, afl->non_instrumented_mode);
  ...
  if (!afl->non_instrumented_mode && !afl->fsrv.qemu_mode &&
    ...
    if (map_size < new_map_size) {
      ...
      afl->fsrv.trace_bits =
          afl_shm_init(&afl->shm, new_map_size, (afl->bb_map_size + 7) / 8, afl->non_instrumented_mode);
